...
DOCKER_CI=ghcr.io/someone-stole-my-name/yaml.nvim-ci:latest
...
docker-%:
	docker run \
		...
		-v $(shell pwd):/data \
		-w /data \
		$(DOCKER_CI) bash -c "export SHELL=bash; apk add yaml-dev && rm -rf /root/.local/state && make packer && make $*"
